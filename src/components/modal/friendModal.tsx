    ...
    const getUsers = async () => {
        try {
            ...
            if (currentUser?.username) {
                usernames = usernames.filter((el) => el !== currentUser.username)
            }
    ...
    const addFriend = async (username: string) => {
        ...
        if (currentUser?.email) {
            ...
            try {
                ...
                toast.error(`Failed to add ${username} as a friend, please try again later!`)
            }
        }
    }
        ...
        <FlexBox direction="column" padding="0 1rem">
            ...
            <FlexBox direction="column" css={{ overflow: 'scroll' }} height='30rem'>
                {userList.map((value, i) => {
                    return (
                        <FlexBox direction="row" justifyContent="space-between" margin="0.5rem 0" alignContent="center" padding="0.5rem" borderRadius="10px" key={i} css={{ border: '2px solid white' }}>
                            <Text fontSize="1.2em" color={COLORS.gray}>{value}</Text>
                            <Button margin="0 0 0 1rem" onClick={() => addFriend(value)} background="none"><img src={Icons.AddUser} alt="" /></Button>
                        </FlexBox>
                    )
                })}
            </FlexBox>
