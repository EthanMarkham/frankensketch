...
const InfiniteScroll = () => {
  ...
  return (
    <>
      <Text color={COLORS.secondaryGreen}>Let's see what others have created!</Text>
      {gameList.map((g, i) => {
        if (gameList.length === i + 1) {
          return <FlexBox width='100%' ref={lastGameRef} key={i}><GameCard game={g} key={g.id} /></FlexBox>
        } else {
          return <FlexBox width='100%' key={i}><GameCard game={g} key={g.id} /></FlexBox>
        }
